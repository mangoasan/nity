...
export default function ScheduleClient() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const title = locale === 'ru'
        ...
        : locale === 'kk'
        ? slot.classType?.titleKk
        : slot.classType?.titleEn;
      setSuccessMsg(`✓ ${title ?? ''} · ${formatDate(date, locale === 'kk' ? 'ru-RU' : `${locale}-${locale.toUpperCase()}`)}`);
  ...
  const getTitle = (slot: ScheduleSlot) =>
    locale === 'ru'
      ? slot.classType?.titleRu
      : locale === 'kk'
      ? slot.classType?.titleKk
  ...
  return (
    <div>
      ...
      <div className="mb-5 text-sm text-[#6b6b6b]">
        {t('nearestClass')}: <span className="text-[#1a1a1a] font-medium">{formatDate(nextDate, locale === 'kk' ? 'ru-RU' : `${locale}-${locale.toUpperCase()}`)}</span>
      </div>
      ...
      <div className="space-y-3">
        {activeDaySlots.length === 0 ? (
          <div className="text-[#6b6b6b] py-8 text-center">—</div>
        ) : (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 sm:hidden">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-base truncate" style={{ fontFamily: 'Georgia, serif', fontWeight: 400 }}>
                        {title}
                      </div>
                  ...
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    ...
                    {isBooked ? (
                      <span className="px-3 py-1.5 rounded-full text-xs" style={{ background: '#E8DCC4', color: '#1a1a1a' }}>
                        ✓ {t('booked')}
                      </span>
                    ) : isFull ? (
                      <span className="px-3 py-1.5 rounded-full text-xs" style={{ background: '#f5e8e8', color: '#c00' }}>
                        {t('full')}
                      </span>
                    ) : (
                      <button
                        ...
                        disabled={isLoading}
                        className="px-4 py-1.5 rounded-full text-xs text-white transition-opacity hover:opacity-90 disabled:opacity-50 min-h-[34px]"
                ...
                <div className="hidden sm:flex sm:items-center gap-4">
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="text-lg truncate" style={{ fontFamily: 'Georgia, serif', fontWeight: 400 }}>
                        {title}
                      </div>
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    <div className="text-sm text-[#6b6b6b]">{filled} / {slot.capacity}</div>
                    {isBooked ? (
                      <span className="px-4 py-2 rounded-full text-sm" style={{ background: '#E8DCC4', color: '#1a1a1a' }}>
                        ✓ {t('booked')}
                      </span>
                    ) : isFull ? (
                      <span className="px-4 py-2 rounded-full text-sm" style={{ background: '#f5e8e8', color: '#c00' }}>
                        {t('full')}
                      </span>
                    ) : (
                      <button
                        onClick={() => handleBook(slot)}
                        disabled={isLoading}
                        className="px-5 py-2 rounded-full text-sm text-white transition-opacity hover:opacity-90 disabled:opacity-50 min-h-[40px]"
                        style={{ background: '#4978BC' }}
                      >
                        {isLoading ? '...' : t('book')}
                      </button>
      ...
      {pendingSlot && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) setPendingSlot(null); }}
        >
          <div className="w-full max-w-sm rounded-2xl shadow-xl" style={{ background: '#fff' }}>
            ...
            <div className="px-6 py-5 space-y-4">
              {/* Class info */}
              <div className="p-4 rounded-xl space-y-2" style={{ background: '#F5F0E8' }}>
                <div className="text-base" style={{ fontFamily: 'Georgia, serif', fontWeight: 400 }}>
                  {getTitle(pendingSlot)}
                </div>
                <div className="space-y-1 text-sm text-[#6b6b6b]">
                  <div className="flex gap-2">
                    <span className="text-[#9b9b9b] min-w-[80px]">{t('confirmDate')}:</span>
                    <span className="text-[#1a1a1a] font-medium">
                      {formatDate(pendingDate, locale === 'kk' ? 'ru-RU' : `${locale}-${locale.toUpperCase()}`)}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <span className="text-[#9b9b9b] min-w-[80px]">{t('confirmTime')}:</span>
                    <span className="text-[#1a1a1a]">{pendingSlot.startTime} – {pendingSlot.endTime}</span>
                  </div>
                  {pendingSlot.master && (
                    <div className="flex gap-2">
                      <span className="text-[#9b9b9b] min-w-[80px]">{t('confirmMaster')}:</span>
                      <span className="text-[#1a1a1a]">{pendingSlot.master.name}</span>
                    </div>
                  )}
                  <div className="flex gap-2">
                    <span className="text-[#9b9b9b] min-w-[80px]">{t('confirmSpots', { count: pendingSpotsLeft })}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 rounded-full text-sm text-white transition-opacity hover:opacity-90"
                  style={{ background: '#4978BC' }}
                >
                  {t('confirmBtn')}
                </button>
